...
Module containing objects useful for dealing with interferometric visibility data.
"""
...
class Data(BaseData):
    ...
    def __init__(self, fn, dtype, coords=None, template=None, m=None):
        """
        Initialize the Data object. Creates a file (or opens an existing file to 
        overwrite). It will optionally also create the file structure based on the 
        structure given in an existing Data object. Otherwise, this object must
        be initialized by reading data from an external data source. Use one of
        the helper functions in this method to load data from the external file.
    ...
    def _open_existing_file(self, fn, template=None):
        """
        Opens an existing file and checks to make sure that the coordinates object
        matches. Also checks to make sure that the template matches, if given.
        """
        self.f = _create_file(fn)
        ...
        for spw in self.f.iterkeys():
            ...
            if template is not None:
                ...
                if d.shape != t.shape:
                    raise Exception('Dataset and template have different shapes!')
        
    def _init_file_from_existing(self, skeleton):
        """
        Initializes a file using the same structure as in an existing Data object.
        Also checks to see if the coordinates object is OK.
        
        Input:
            skeleton - [Data] - Existing Data object to use as a template for ini-
                tializing the file for this object.
        ...
        for i in skeleton.f.iterkeys():
            ...
            g.create_dataset(DATASET_STRING, \
                shape=skeleton.f[i][DATASET_STRING].shape, dtype=self.dtype)
        ...
        for spw in self.f.iterkeys():
            ...
            for chan in range(ds.len()):
                ...
                if othertype == DO:
                    ...
                    if len(other) != len(rs):
                        raise Exception("Array size does not match number of "+\
...
class PolData(BaseData):
    ...
    def store_records(self, a, spw, chan):
        ...
        if not type(a) is list and len(a) != 2:
            raise TypeError("Must pass a length 2 list of arrays to store in "+\
                "PolData object.")
...
#def read_pol_data_from_ms(msfn, vis, noise, viscol="DATA", \
#    noisecol='SIGMA'):
#        """
#        Reads polarization data (Q, U) into a visibility and a noise data class 
#        from a MeasurementSet file.
#        
#        Args:
...
#            m: Messenger object to use for displaying messages to stdout. If None, 
#                default messages will be displayed. [None]
#        
#        Returns:
#            Nothing.
#        """
...
#            return
#        
#        m.header2("Reading data from the MeasurementSet...")
#        
#        
...
#        
#        # just the Q,U part of the S Jones matrix (hence Spart)
...
#                
#                data_recs = data_recs.reshape(nrecs_stab,nchan)
#                noise_recs = noise_recs.reshape(nrecs_stab)
#                flag_recs = flag_recs.reshape(nrecs_stab,nchan)
#
#                
...
def read_data_from_ms(msfn, vis, noise, viscol="DATA", \
    noisecol='SIGMA', mode='pol'):
        """
